...
- **No migrate or rollback CLI verb, and no implemented operator verb.** In
  ...
  `bootstrap` and the rest are advertised by `--help` and dispatched (task
  I-003) but still answer `NOT_READY` (exit 4), and the daemon rejects them as
  unrecognised commands. There is no command that reads a journal, resumes a
  cutover or rolls anything back.
