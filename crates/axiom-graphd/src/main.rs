//! `axiom-graphd` binary entry point.
//!
...
use axiom_graphd::cli;
use axiom_graphd::telemetry::{LevelFilter, LogLevel, Telemetry};

fn main() -> ProcessExitCode {
    let arguments: Vec<String> = std::env::args().skip(1).collect();
    let invocation = cli::parse(&arguments);
    let mut telemetry = Telemetry::stderr(LevelFilter::at_least(match invocation.json() {
        true => LogLevel::Debug,
        false => LogLevel::Info,
    }));
    let mut stdout = std::io::stdout();
    let exit_code = cli::run(&invocation, &mut stdout, &mut telemetry);
...
fn exit_status(exit_code: axiom_graphd::cli::ExitCode) -> u8 {
